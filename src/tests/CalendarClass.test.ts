...
describe("CalendarClass", () => {
    ...
    test("calendar createTask adds new task to localStorage", async () => {
        ...
        await calendar.createTask(new Date("2023-11-10"), "doing js exercise", ["programming", "js"])
        tasks = JSON.parse(localStorage.getItem("tasks") as string);
        expect(tasks).toHaveLength(1);
    });

    test("calendar readTask returns task with specified ID", async () => {
        const taskId = await calendar.createTask(new Date("2023-11-10"), "doing js exercise", ["programming", "js"])
    ...
    test("calendar updateTask updates task according to passed params", async () => {
        ...
        await calendar.updateTask(taskId, new Date('2023-11-12'), 'learn english', ['languages'],  "active");
    ...
    test("calendar filterTask filters task according to passed params", async () => {
        ...
        const task1 = await calendar.createTask(new Date("2023-11-10"), "doing js exercise", ["programming", "js"])
        const task2 = await calendar.createTask(new Date("2023-11-20"), "learn english", ["languages"])
        const task3 = await calendar.createTask(new Date("2023-11-30"), "fitness training", ["fitness", "GYM"])
        ...
        //filter by text
        filteredTasks = await calendar.filterTasks({tasksArray: tasks,
                                                text: "english"});
        ...
        //filter by status
        await calendar.updateTask(task1, new Date("2023-11-10"), "doing js exercise", ["programming", "js"], 'done')
